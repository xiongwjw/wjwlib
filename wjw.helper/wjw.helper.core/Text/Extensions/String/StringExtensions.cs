...
namespace wjw.helper.Text
{
    ...
    public  static  partial class StringExtensions
    {
        ...
        public static IEnumerable<string> ExtractArguments(this string value, string template,
            ComparsionTemplateOptions compareTemplateOptions = ComparsionTemplateOptions.Default,
        ...
        {
            template = template.ReplaceAll(new[] { @"\", "^", "$", "*", "+", "?", ".", "(", ")" }, v => @"\" + v);
            //前缀比较
            ...
            sb.Append(Regex.Replace(template, @"\{[0-9]+\}", delegate(Match m)
            {
        ...
        public static string ConvertBinary(this string value, BinaryMode from, BinaryMode to)
        {
            try
            {
                int fromTarget = GetBinary(from);
                int toTarget = GetBinary(to);
                int intValue = Convert.ToInt32(value, fromTarget);//先转换成10进制
                string result = Convert.ToString(intValue, toTarget);//再转成目标进制
                if (toTarget == 2)
                {
        ...
        private static int GetBinary(BinaryMode mode)
        {
            switch (mode)
            {
                case BinaryMode.Binary:
                    return 2;
                case BinaryMode.Octonary:
                    return 8;
                case BinaryMode.DecimalBase:
                    return 10;
                case BinaryMode.Hex:
                    return 16;
                default:
        ...
        #region 字符串安全
        #region ToJsEscapeString(Js字符串转义)
        /// <summary>
        /// js字符串转义，Javascripte中JsEscape编码服务端实现
        /// </summary>
        /// <param name="source">js字符串</param>
        /// <returns></returns>
        //public static string ToJsEscapeString(this string source)
        //{
        //  //  return GlobalObject.escape(source);
        //}
        #endregion
